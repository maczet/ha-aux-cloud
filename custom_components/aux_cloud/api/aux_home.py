...
def _decode_running_hex(running_hex: str) -> dict:
    ...
    def _byte(index: int, default: int = 0) -> int:
        """Return b[index] or *default* when the payload is too short."""
    ...
    # b[31] is absent on the minimal 27-byte payload; default its nibble to 0.
    params["envtemp"] = (b[15] - 0x20) * 10 + (_byte(31) & 0x0F)

    # Eco mode: byte[11] bit 7.
    # WARNING: bit 7 is also part of the ac_mode mask (0xE0) above — 0x80 = HEAT.
    # This means HEAT currently always decodes as ecomode=1, and eco-on in COOL
    # (0xA0) falls outside the mode map and defaults to auto.  The real eco flag
    # is likely on a different byte; confirm against a live capture (see
    # scripts/probe_running_hex.py) before trusting this value.
...
class AuxHomeAPI:
    ...
    async def _make_request(
        ...
        try:
            ...
            raise AuxHomeApiError(
                f"AUX Home: network error calling {endpoint}: {exc}"
            ) from exc
    ...
    async def query_device_state(self, device_id: str, product_key: str, password: str = "") -> dict:
        ...
        try:
            ...
            code = json_data.get("code")
            if code not in (0, "0", 200, "200"):
                # commonQuery is frequently rejected by the server (e.g. 500);
                # get_devices() provides the authoritative state, so this is not
                # an error — keep it at debug to avoid log spam every poll.
                _LOGGER.debug(
    ...
    async def set_device_params(self, device: dict, values: dict):
        ...
        """
        # ac_mode already carries the AUX-Home-native air_con_func value: this
        # fork's api/const.py defines AC_MODE_* as {cool:1, heat:4, dry:2, fan:6,
        # auto:0}, and _decode_running_hex emits the same convention.  No further
        # translation is needed — passing ac_mode straight through as air_con_func.
        # (An earlier _CLOUD_TO_AUX_HOME_MODE map assumed the old AUX Cloud
        # convention and double-translated, so COOL was being sent as HEAT.)

        # wind_speed values are IDENTICAL to the internal ac_mark values, so the
        # fan speed is passed through unchanged (as a string, per the API).
        ...
        for key, val in values.items():
            if key == "temp":
                ...
                # ac_mode is already the AUX Home air_con_func value (APK-confirmed key).
                intent["air_con_func"] = int(val)
                _LOGGER.debug("AUX Home: ac_mode %s → air_con_func %s", val, val)
            elif key == "ac_mark":
                # Fan speed maps 1:1 onto wind_speed; sent as string per the API.
                intent["wind_speed"] = str(int(val))
                _LOGGER.debug("AUX Home: ac_mark %s → wind_speed %s", val, val)
            else:
        ...
        for single_intent in intents_to_send:
            ...
            if code not in (0, "0", 200, "200"):
                ...
                _LOGGER.debug("AUX Home: control success code=%s intent=%s", code, single_intent)
                any_success = True
